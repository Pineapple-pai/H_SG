...
{
    // 直接参考:
    // 1. F:\H_SG\MDK-ARM\User\Algorithm\ChassisCalculation\StringWheel.hpp
    //    使用 Alg::CalculationBase::String_IK::Set_current_steer_angles(angle, index)
    //    使用 Alg::CalculationBase::String_IK::StringInvKinematics(vx, vy, vw, phase, speed_gain, rotate_gain)
    //    使用 Alg::CalculationBase::String_IK::GetMotor_wheel(index)
    //    使用 Alg::CalculationBase::String_IK::GetMotor_direction(index)
    // 2. 当前舵向角可从 F:\H_SG\MDK-ARM\User\BSP\Motor\Lk\Lk_motor.hpp 的
    //    BSP::Motor::LK::Motor4005.getAngleRad(i + 1) 读取。
    // TODO(教学挖空): 在这里完成底盘速度到四个舵轮目标的解算。
    // 提示:
    // 1. 输入是 Chassis_Data.vx / vy / vw。
    // 2. 需要结合舵向电机当前角度，完成逆解并得到每个轮子的目标速度和目标角度。
    // 3. 最终把结果写回 Chassis_Data.tar_speed[i] 和 Chassis_Data.tar_angle[i]。
    // 提示词:
    // "请补全四舵轮底盘逆运动学: 根据 vx、vy、vw 和当前舵向角，求每个轮子的目标转速与目标角度。"
    for (int i = 0; i < 4; ++i)
    {
        Chassis_Data.tar_speed[i] = 0.0f;
        Chassis_Data.tar_angle[i] = 0.0f;
    }
}
...
{
    // 直接参考:
    // 1. 实际轮速可从 F:\H_SG\MDK-ARM\User\BSP\Motor\Dji\DjiMotor.hpp 的
    //    BSP::Motor::Dji::Motor3508.getVelocityRads(i + 1) 读取。
    // 2. 滤波对象直接用 td_3508_speed[i].Calc(feedback_speed)。
    // TODO(教学挖空): 在这里完成轮速反馈滤波。
    // 提示:
    // 1. 读取每个驱动轮电机的实际转速反馈。
    // 2. 使用已有 TD / 滤波器对象更新 td_3508_speed[i]。
    // 3. 后续 PID_Updata 会直接使用 td_3508_speed[i].x1。
    // 提示词:
    // "请补全驱动轮速度反馈滤波: 读取电机实际转速并更新 td_3508_speed。"
    for (int i = 0; i < 4; ++i)
    {
        td_3508_speed[i].Calc(0.0f);
    }
}
...
{
    // 直接参考:
    // 1. PID 接口在 F:\H_SG\MDK-ARM\User\Algorithm\PID.hpp
    //    使用 PID::GetPidPos(...)、PID::GetCout()、PID::clearPID()。
    // 2. 舵向反馈可用 BSP::Motor::LK::Motor4005.getAngleDeg(i + 1)
    //    和 BSP::Motor::LK::Motor4005.getVelocityRpm(i + 1)。
    // 3. 驱动轮速度环可直接使用 td_3508_speed[i].x1 作为反馈。
    // 4. 舵向目标角平滑可继续调用本文件里的 SmoothSteerTarget(...)。
    // TODO(教学挖空): 在这里补全舵向电机和驱动轮电机的 PID 计算。
    // 提示:
    // 1. 舵向通常是角度环 + 速度环串级。
    // 2. 驱动轮通常直接做速度环。
    // 3. 计算结果应保存在 pid_angle_String / pid_vel_String / pid_vel_Wheel 中。
    // 提示词:
    // "请补全四舵四驱底盘控制环: 舵向用串级 PID，驱动轮用速度环 PID，并写回各 PID 对象输出。"
    for (int i = 0; i < 4; ++i)
    {
        pid_angle_String[i].clearPID();
        pid_vel_String[i].clearPID();
        pid_vel_Wheel[i].clearPID();
        Chassis_Data.final_3508_Out[i] = 0.0f;
        Chassis_Data.final_4005_Out[i] = 0.0f;
    }
    return;
    // 先算舵向双环：角度环输出给速度环。
    for (int i = 0; i < 4; i++)
...
{
    // 直接参考:
    // 1. 原始 PID 输出可通过 pid_vel_Wheel[i].GetCout() / pid_vel_String[i].GetCout() 取得。
    // 2. 功率分配接口在本工程 PowerControl 对象中，常用:
    ...
    // TODO(教学挖空): 在这里把 PID 输出整理成最终电机输出，并加入功率限制。
    // 提示:
    // 1. 先从 pid_vel_Wheel / pid_vel_String 取出原始输出。
    // 2. 再根据功率控制模块，对输出做限幅或重分配。
    // 3. 最后把结果写到 Chassis_Data.final_3508_Out / final_4005_Out。
    // 4. 若需要发 CAN 目标值，可在这里写入各电机发送缓冲区。
    // 提示词:
    // "请补全底盘输出分配: 读取 PID 输出，结合功率限制做二次处理，生成最终的电机控制量。"
    for (int i = 0; i < 4; ++i)
    {
        Chassis_Data.final_3508_Out[i] = 0.0f;
        Chassis_Data.final_4005_Out[i] = 0.0f;
...
{
    // 直接参考:
    // 1. 舵向电机发送接口: F:\H_SG\MDK-ARM\User\BSP\Motor\Lk\Lk_motor.hpp
    //    BSP::Motor::LK::Motor4005.ctrl_Multi(iq_control)。
    // 2. 驱动轮发送接口: F:\H_SG\MDK-ARM\User\BSP\Motor\Dji\DjiMotor.hpp
    //    BSP::Motor::Dji::Motor3508.sendCAN()。
    // 3. 超级电容发送接口:
    //    BSP::SuperCap::cap.sendCAN()。
    // TODO(教学挖空): 在这里发送电机和超级电容的 CAN 指令。
    // 提示:
    // 1. 舵向电机和驱动轮电机可能分帧或分时发送。
    // 2. 发送前应先把 final_3508_Out / final_4005_Out 做适当限幅。
    // 3. 如果你们的超级电容需要周期下发控制指令，也可以在这里处理。
    // 提示词:
    // "请补全底盘 CAN 下发流程: 将最终控制量写入电机并按既定节奏发送。"
    Send_ms++;
    Send_ms %= 2;
    return;
